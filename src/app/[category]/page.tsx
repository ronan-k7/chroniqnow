...
export async function generateMetadata({
  ...
  const ogImage = latest?.image ?? "https://www.chroniqnow.com/images/chroniqnow-logo.webp";
  ...
  return (
    <>
      ...
      <main className="p-2 sm:p-20 py-8 min-h-screen">
        ...
        <nav aria-label="Breadcrumb" className="mb-4 text-sm font-bold text-gray-500">
        ...
        <section className="mb-10 text-gray-700 leading-relaxed max-w-4xl">
          <p className="mb-2">
            Follow the most important <strong>{capitalized} news</strong> stories as they break, 
        ...
        <div className="hidden lg:grid grid-cols-5 gap-6 mb-12">
          ...
          <div className="col-span-3 grid grid-cols-3 gap-6">
             {/* Small Card 1 */}
             <Link href={`/${category}/${otherPageArticles[0].slug}`} className="group shadow-sm hover:shadow-md transition">
              <div className="aspect-video overflow-hidden">
                <Image src={otherPageArticles[0].image} alt="" width={400} height={250} className="object-cover w-full h-full group-hover:scale-105 transition duration-300" />
              </div>
              <h3 className="p-3 text-sm font-bold line-clamp-3">{otherPageArticles[0].title}</h3>
            </Link>

            {/* Main Featured Card (H2) */}
            <Link href={`/${category}/${featuredArticle.slug}`} className="col-span-2 flex flex-col group shadow-md hover:shadow-lg transition">
              ...
              <div className="p-6">
                <h2 className="text-2xl font-bold mb-3 group-hover:text-red-600 transition">{featuredArticle.title}</h2>
                <p className="text-sm text-gray-600 line-clamp-3">{featuredArticle.shortdescription}</p>
              </div>
            </Link>

            {/* Small Card 2 */}
            <Link href={`/${category}/${otherPageArticles[1].slug}`} className="group shadow-sm hover:shadow-md transition">
              <div className="aspect-video overflow-hidden">
                <Image src={otherPageArticles[1].image} alt="" width={400} height={250} className="object-cover w-full h-full" />
              </div>
              <h3 className="p-3 text-sm font-bold line-clamp-3">{otherPageArticles[1].title}</h3>
            </Link>
          </div>
          ...
          <div className="col-span-2 space-y-4">
            {otherPageArticles.slice(2, 6).map((item) => (
              <Link key={item.slug} href={`/${category}/${item.slug}`} className="flex gap-4 items-center shadow-sm hover:bg-gray-50 transition p-2">
                <div className="w-24 h-24 flex-shrink-0 overflow-hidden">
                   <Image src={item.image} alt="" width={100} height={100} className="object-cover w-full h-full" />
                </div>
        ...
        <div className="lg:hidden space-y-8">
          <Link href={`/${category}/${featuredArticle.slug}`} className="block">
            <Image src={featuredArticle.image} alt={featuredArticle.title} width={600} height={400} className="w-full aspect-video object-cover mb-4 shadow-sm" priority />
            <h2 className="text-2xl font-bold mb-2">{featuredArticle.title}</h2>
            <p className="text-gray-600 text-sm">{featuredArticle.shortdescription}</p>
          </Link>
          
          <div className="space-y-6">
            {otherPageArticles.slice(0, 6).map((item) => (
              <Link key={item.slug} href={`/${category}/${item.slug}`} className="flex gap-4">
                <div className="w-20 h-20 flex-shrink-0">
                  <Image src={item.image} alt="" width={80} height={80} className="object-cover w-full h-full shadow-xs" />
                </div>
        ...
        {bottomArticles.length > 0 && (
          <section className="mt-16 border-t pt-12">
            <h2 className="text-xl font-bold mb-8 uppercase text-gray-500 tracking-widest">Archive News</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {bottomArticles.map((item) => (
                <Link key={item.slug} href={`/${category}/${item.slug}`} className="group">
                  <div className="aspect-video overflow-hidden mb-3">
                    <Image src={item.image} alt="" width={400} height={250} className="object-cover w-full h-full group-hover:scale-105 transition" />
                  </div>
